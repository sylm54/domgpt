...
export const Chat = forwardRef<HTMLDivElement, ChatProps>(function Chat(
	...
	useEffect(() => {
		const unsubscribe = agent.context.listen((ctx) => {
			...
			setStatus((prev) =>
				ctx.in_progress ? "streaming" : prev === "submitted" ? "submitted" : "idle"
			);
		});
	...
	const renderMessageParts = (
		...
		return parts.map((part, idx) => {
			...
			if (part.type === "tool") {
				...
				const hasOutput = typeof part.tool_output !== "undefined" && part.tool_output !== null;
	...
	const renderChatMessage = (msg: ChatMessage, index: number, streaming = false) => {
							...
							{msg.type === "event" ? (
								...
								<div className="grid gap-2">
									...
									{"stats" in msg && msg.stats ? (
										<div className="text-xs text-muted-foreground mt-1" aria-hidden>
	...
	return (
		<div ref={ref} className={cn("relative flex h-full flex-col min-h-0 flex-1", className)}>
			...
			>
				<div className="flex h-full flex-col">
					<div className="flex items-center justify-between border-b border-primary/20 bg-primary/5 px-4 py-3 mt-20">
						<h2 className="text-sm font-semibold text-primary">System Messages</h2>
				...
				>
					...
					<PromptInputToolbar className="border-t border-primary/10 bg-primary/5 rounded-b-2xl">
						...
						<div className="flex items-center gap-2 pr-3">
							<div aria-hidden>{status === "streaming" ? <Loader /> : null}</div>
							<PromptInputSubmit
								status={
									status === "streaming"
										? "streaming"
